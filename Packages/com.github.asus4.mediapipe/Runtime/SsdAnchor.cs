...
{
    public struct SsdAnchor : IEquatable<SsdAnchor>
    {
        ...
        public static bool operator ==(SsdAnchor lhs, SsdAnchor rhs)
        {
            ...
            var diffw = lhs.width - rhs.width;
            var diffh = lhs.height - rhs.height;
            var sqrmag = diffx * diffx + diffy * diffy + diffw * diffw + diffw * diffw;
    ...
    public static class SsdAnchorsCalculator
    {
        ...
        public static SsdAnchor[] Generate(Options options)
        {
            ...
            while (layer_id < options.strides.Length)
            {
                ...
                {
                    var scale = CalculateScale(options.minScale,
                        options.maxScale,
                    ...
                    {
                        for (var aspect_ratio_id = 0;
                             aspect_ratio_id < options.aspectRatios.Length;
                             ++aspect_ratio_id)
                        {
                        ...
                        {
                            var scale_next = last_same_stride_layer == options.strides.Length - 1
                                ? 1.0f
                ...
                var feature_map_height = 0;
                var feature_map_width = 0;
                ...
                {
                    var stride = options.strides[layer_id];
                ...
                for (var anchor_id = 0; anchor_id < anchor_height.Count; ++anchor_id)
                {
                    ...
                    var x_center = (x + options.anchorOffsetX) * 1.0f / feature_map_width;
                    var y_center = (y + options.anchorOffsetY) * 1.0f / feature_map_height;
