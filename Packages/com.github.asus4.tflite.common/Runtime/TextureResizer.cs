...
namespace TensorFlowLite
{
    ...
    /// </summary>
    [Obsolete("TextureResizer is obsolete, use TextureToNativeTensor instead")]
    public sealed class TextureResizer : IDisposable
    {
        private static readonly int _VertTransform = Shader.PropertyToID("_VertTransform");
        ...
        public Material material
        {
            get
            {
                if (_blitMaterial == null) _blitMaterial = new Material(Shader.Find("Hidden/TFLite/Resize"));
        ...
        {
            VertexTransform =
                GetVertTransform(options.rotationDegree, options.mirrorHorizontal, options.mirrorVertical);
        ...
        private RenderTexture ApplyResize(Texture texture, int width, int height, bool fillBackground)
        {
            ...
            {
                DisposeUtil.TryDispose(this.texture);
                this.texture = new RenderTexture(width, height, 0, RenderTextureFormat.ARGB32);
            }

            if (fillBackground)
                // Fill with color 0,0,0,0
                Graphics.Blit(Texture2D.blackTexture, this.texture);

            Graphics.Blit(texture, this.texture, material, 0);
            return this.texture;
        }
