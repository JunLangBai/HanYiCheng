...
namespace TensorFlowLite
{
    ...
    public class FilePopupDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (selectedIndex < 0) selectedIndex = FindSelectedIndex(property.stringValue);
        ...
        private void InitDisplayNames(string regex)
        {
            ...
            var fullPaths = Directory.GetFiles(Application.streamingAssetsPath, regex, SearchOption.AllDirectories);

            displayNames = fullPaths.Select(f =>
            {
                var path = f.Replace(Application.streamingAssetsPath, "")
                    .Replace('\\', '/');
