...
namespace LeTai.Asset.TranslucentImage
{
    public class ScalableBlur : IBlurAlgorithm
    {
        ...
        {
            var radius = ScaleWithResolution(config.Radius,
                target.width * srcCropRegion.width,
                target.height * srcCropRegion.height);
            ConfigMaterial(radius, backgroundFill);

            var stepCount = Mathf.Clamp(config.Iteration * 2 - 1, 1, scratches.Length * 2 - 1);
            var extent = config.Strength;
            ...
            if (stepCount > 1)
            {
                CropViewport(target.width >> 1, target.height >> 1, extent, out var viewportFirst,
                    out var activeRegionFirst);
                propertyBlock.SetVector(ShaderID.CROP_REGION,
            ...
            for (var i = 1; i < stepCount; i++)
            {
                ...
                CropViewport(target.width >> targetDepth, target.height >> targetDepth, extent, out var viewportStep,
                    out var activeRegionStep);
            ...
            void CropViewport(int targetWidth, int targetHeight, float padding, out Rect viewport,
                out Rect activeRegionSnapped)
            {
