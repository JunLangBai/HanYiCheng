...
namespace LeTai.Asset.TranslucentImage
{
    ...
    internal unsafe struct ActiveRegionMergeJob : IJob
    {
        [ReadOnly] public NativeList<ActiveRegion> activeRegions;
        [ReadOnly] public NativeList<Matrix4x4> vpMatrices;
        ...
        public void Execute()
        {
            ...
            for (var i = 0; i < activeRegions.Length; i++)
            {
                ...
                for (var j = 0; j < 4; j++)
                {
                    var pointWorld = quad.localToWorld.MultiplyPoint3x4(corners[j]);
                    Vector2 pointScreen;
                    if (quad.IsWorldSpace)
                    {
                        Vector4 pointWorldHomo = pointWorld;
                        pointWorldHomo.w = 1;
                        var pointClip = vpMatrices[quad.vpMatrixCacheIndex.index] * pointWorldHomo;
