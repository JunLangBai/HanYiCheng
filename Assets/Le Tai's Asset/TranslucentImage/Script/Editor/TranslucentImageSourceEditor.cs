...
namespace LeTai.Asset.TranslucentImage.Editor
{
    ...
    {
        private EditorProperty backgroundFill;

        private EditorProperty blurConfig;
        private EditorProperty blurRegion;
        private UnityEditor.Editor configEditor;
        private EditorProperty cullPadding;
        private EditorProperty downsample;
        private EditorProperty maxUpdateRate;
        private EditorProperty preview;
        private EditorProperty skipCulling;

        private ScalableBlurConfigEditor ConfigEditor
        {
            get
            {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!curConfig)
            {
                ...
                {
                    var newConfig = CreateInstance<ScalableBlurConfig>();
                ...
                && skipCulling.serializedProperty.boolValue == false)
            {
                ...
                if (cullPadding.serializedProperty.floatValue <= 0)
                    EditorGUILayout.HelpBox("When using a limited Max Update Rate with Culling enabled," +
                                            " add some padding to prevent culled area being shown during UI movement.",
                        MessageType.Warning);
            }
