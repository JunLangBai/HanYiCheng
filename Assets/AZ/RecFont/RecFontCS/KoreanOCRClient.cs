...
public class KoreanOCRClient : MonoBehaviour
{
    ...
    [Header("画板组件")] public DrawingBoard drawingBoard;

    [Header("UI 组件")] public RawImage targetRawImage; // 要识别的图片
    ...
    [Header("服务器配置")] public string serverURL = "http://110.40.170.159:6666/predict";

    public int timeoutSeconds = 15;

    [Header("测试图片")] public Texture2D testTexture; // 可选：用于测试的图片

    private Coroutine currentRecognitionCoroutine; // 当前识别协程

    private bool isRecognizing; // 识别状态标志
    ...
    {
        // 绑定清除按钮事件
        if (clearButton != null) clearButton.onClick.AddListener(ClearDrawingBoard);

        // 绑定按钮事件
        if (recognizeButton != null) recognizeButton.onClick.AddListener(StartRecognition);

        // 初始化状态
        UpdateResult("识别结果是：");

        // 如果有测试图片，加载到RawImage
        if (testTexture != null && targetRawImage != null) targetRawImage.texture = testTexture;
    }

    private void OnDestroy()
    {
        // 清理协程
        if (currentRecognitionCoroutine != null) StopCoroutine(currentRecognitionCoroutine);
    }
    ...
    private IEnumerator RecognizeImage()
    {
        ...
        // 获取纹理
        var texture = GetTextureFromRawImage();
        if (texture == null)
        {
        ...
        // 发送请求
        using (var request = UnityWebRequest.Post(serverURL, formData))
        {
            // 设置超时时间
            ...
            // 计算请求耗时
            var requestTime = Time.time - startTime;
            Debug.Log($"请求耗时: {requestTime:F2}秒");

            // 检查是否仍在识别状态（防止被取消）
            if (!isRecognizing) yield break;

            // 处理响应
            var success = ProcessResponse(request);
    ...
    private bool ProcessResponse(UnityWebRequest request)
    {
        if (request.result == UnityWebRequest.Result.Success)
            try
            {
                ...
                var response = JsonUtility.FromJson<OCRResponse>(responseText);

                if (!string.IsNullOrEmpty(response.label))
                {
                    UpdateResult($"识别结果是: {response.label}");
                    Debug.Log($"OCR识别结果: {response.label}");
                    if (OCR_UIManager.Instance) OCR_UIManager.Instance.OnRecognitionComplete(true, response.label);
                    return true; // 成功
                }
    ...
    private Texture2D GetTextureFromRawImage()
    {
        if (targetRawImage == null || targetRawImage.texture == null)
            return null;

        // 如果已经是Texture2D，直接返回
        if (targetRawImage.texture is Texture2D) return targetRawImage.texture as Texture2D;
