...
public class UIManager : MonoBehaviour
{
    ...
    // ========== UI元素引用配置 ==========
    [Header("UI Elements")] public TextMeshProUGUI questionText; // 题目文本显示组件
    ...
    {
        foreach (var option in currentOptions) Destroy(option.gameObject);
    ...
    private void UpdateProgress()
    {
        ...
        var total = QuestionManager.Instance.questionData.questions.Count;
    ...
    public string GetSelectedOptionContent()
    {
        ...
        if (option != null) return $"选中内容：{option.text}";
        return "未选择任何选项";
    }
    ...
    {
        var option = QuestionManager.Instance.GetSelectedOption();
        nowChose.text = option != null ? $"当前选择：{option.text}" : "请选择答案";
    }

    public float GetScore()
    {
        var totalQuestions = QuestionManager.Instance.questionData.questions.Count;
