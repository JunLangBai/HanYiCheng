...
public class MainUIManager : MonoBehaviour
{
    ...
    private GameData gameData = JsonFileManager.LoadFromJson<GameData>("GameData.json");
    ...
    private void CheckDailyLogin(ref GameData gameData)
    {
        // 定义中国时区（Windows系统用"China Standard Time"，Linux/macOS用"Asia/Shanghai"）
        var chinaTimeZone = TimeZoneInfo.FindSystemTimeZoneById("China Standard Time");
        var utcTime = DateTime.UtcNow;
        var currentUTC = TimeZoneInfo.ConvertTimeFromUtc(utcTime, chinaTimeZone);
        ...
        // 计算时间差（只比较日期部分）
        var lastDate = gameData.LastLoginUTC.Date;
    ...
    {
        // 检查是否完成了摸底和教程
        if (!gameData.placementClear || !gameData.tutorialClear) return null;
    ...
    private IEnumerator UpdateTime()
    {
        ...
        {
            var utcTime = DateTime.UtcNow;
            var chinaTime = TimeZoneInfo.ConvertTimeFromUtc(utcTime, chinaTimeZone);
