Shader "Unlit/DayNightButton"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float sdRoundedCross(fixed2 p, fixed h, fixed2 center, float size)
            {
                ...
                float k = 0.5 * (h + 1.0 / h);
                p = abs(p);
                return (p.x < 1.0 && p.y < p.x * (k - h) + h) ? k - sqrt(dot2(p - fixed2(1, k))) : 1;
                // sqrt(min(dot2(p - fixed2(0, h)), dot2(p - fixed2(1, 0))))
            }
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                {
                    _BGColor = lerp(_Circle3ColorDay, _Circle3ColorNight,
                                                                     Remap(_SunPos, fixed2(-1.6, 1.6), fixed2(0, 1)));
                }
                else
                {
  ...
  _CloudPos.y + Remap(_SunPos, fixed2(-1.6, 1.6), fixed2(0, -2))),
                ...
                // 1-4
                float star1 = sdRoundedCross(i.uv, _StarsLen.x, fixed2(_StarsPosX.x, _StarsPosY.x), _StarsSize.x) -
                    _StarsRound.x;
       ...
       clamp(
           ...
           0.6, 1), fixed2(0.6, 1), fixed2(0, 1)));
                ...
                col = lerp(col, _StarColor,
                                                                     Remap(
                                                                         clamp(
                                                                             1.0 - smoothstep(0.0, 0.1, abs(star5)) - (
                                                                                 _StarsStrenthG2.x *
                                                                                 sign(i.uv.x - _StarsPosXG2.x) * sign(
                                                                                     i.uv.y - _StarsPosYG2.x) *
                                                                                 dot2Offset(
                                                                                     i.uv, fixed2(
                                                                                         _StarsPosXG2.x,
                                                                                         _StarsPosYG2.x))),
                ...
                col = lerp(col, _StarColor,
                                                                      Remap(
                                                                          clamp(
                                                                              1.0 - smoothstep(0.0, 0.1, abs(star6)) - (
                                                                                  _StarsStrenthG2.y *
                                                                                  sign(i.uv.x - _StarsPosXG2.y) * sign(
                                                                                      i.uv.y - _StarsPosYG2.y) *
                                                                                  dot2Offset(
                                                                                      i.uv, fixed2(
                                                                                          _StarsPosXG2.y,
                                                                                          _StarsPosYG2.y))), 0.6, 1),
                                                                          fixed2(0.6, 1),
                                                                          fixed2(0, 1)));
                ...
                col = lerp(col, lerp(_HighLightColor, _HighLightNightColor,
                                     Remap(_SunPos, fixed2(-1.6, 1.6), fixed2(0, 1))),
                           saturate(sunShadowLight1 - sunShadowLight2));
