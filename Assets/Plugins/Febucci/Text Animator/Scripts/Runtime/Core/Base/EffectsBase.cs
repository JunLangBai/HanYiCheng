...
namespace Febucci.UI.Core
{
    ...
    ///     Please do not inherit from this class directly, but do from <see cref="AppearanceBase" /> or
    ///     <see cref="BehaviorBase" />
    /// </summary>
    public abstract class EffectsBase
    {
        ...
        /// </summary>
        /// <remarks>
        ///     Multiply this by your effect values only if they behave differently with different screen resolutions, font sizes
        ///     or similar. (e.g. adding or subtracting vectors)
        /// </remarks>
        ...
        {
            if (FormatUtils.ParseFloat(modifierValue, out var multiplier)) value *= multiplier;
        }
        ...
        internal class RegionManager
        {
            ...
            {
                var text = $"{entireRichTextTag} - {textRegions.Count} region(s): ";
        ...
        private List<Modifier> modifiers { get; } = new();
        ...
        {
            for (var i = 0; i < modifiers.Count; i++) SetModifier(modifiers[i].name, modifiers[i].value);
        }
#endif
