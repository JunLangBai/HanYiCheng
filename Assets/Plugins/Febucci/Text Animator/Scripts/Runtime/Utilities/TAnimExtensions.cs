...
namespace Febucci.UI.Core
{
    internal static class TAnimExtensions
    {
        ...
        {
            for (var a = effects.Count - 1;
                 a >= 0;
                 a--) //closes the last recurring region, leaving eventual fallback/default effects unaltered
        ...
        internal static bool CloseElement<T>(this List<T> effects, int listIndex, int realTextIndex)
            where T : EffectsBase
        {
        ...
        internal static bool CloseRegionNamed<T>(this List<T> effects, string endTag, int realTextIndex)
            where T : EffectsBase
        {
        ...
        internal static bool CloseSingleOrAllEffects<T>(this List<T> effects, string closureTag, int realTextIndex)
            where T : EffectsBase
        {
