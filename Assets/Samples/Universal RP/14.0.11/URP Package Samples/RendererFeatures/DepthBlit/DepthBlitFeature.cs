...
public class DepthBlitFeature : ScriptableRendererFeature
{
    ...
    private DepthOnlyPass
        m_DepthOnlyPass; // DepthOnlyPass is for platforms that run OpenGL ES, which does not support CopyDepth.
    ...
    {
        var msaaEnabledForCamera = cameraData.cameraTargetDescriptor.msaaSamples > 1;
        var supportsTextureCopy = SystemInfo.copyTextureSupport != CopyTextureSupport.None;
        var supportsDepthTarget = RenderingUtils.SupportsRenderTextureFormat(RenderTextureFormat.Depth);
        var supportsDepthCopy = !msaaEnabledForCamera && (supportsDepthTarget || supportsTextureCopy);

        var msaaDepthResolve = msaaEnabledForCamera && SystemInfo.supportsMultisampledTextures != 0;
    ...
    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        ...
        else
        {
            if (m_DepthOnlyPass == null)
                m_DepthOnlyPass =
                    new DepthOnlyPass(evt_Depth, RenderQueueRange.opaque, rendererDataAsset.opaqueLayerMask);
    ...
    public override void SetupRenderPasses(ScriptableRenderer renderer, in RenderingData renderingData)
    {
        ...
        desc.msaaSamples = 1;
        RenderingUtils.ReAllocateIfNeeded(ref m_DepthRTHandle, desc, FilterMode.Bilinear, TextureWrapMode.Clamp,
            name: k_DepthRTName);
