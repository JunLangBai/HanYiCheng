...
namespace Rokid.UXR.Demo
{
    public class HeadTrackingSample : AutoInjectBehaviour
    {
        ...
        [Autowrited] private Text engineTxt;

        [Autowrited] private Text infoTxt;

        [Autowrited] private Toggle mixThreeDof;

        [Autowrited] private Button recenterMixThreeDof;

        [Autowrited] private Button sixDofButton;

        [Autowrited] private Text slamImageQuality;

        [Autowrited] private Text slamKineticQuality;

        [Autowrited] private Text slamTrackingStatus;
        ...
        private void Start()
        {
            ...
            if (mainCamera == null) mainCamera = MainCameraCache.mainCamera;
            ...
            mixThreeDof.onValueChanged.AddListener(isOn => { cameraRig.mixThreeDof = isOn; });

            recenterMixThreeDof.onClick.AddListener(() => { MixThreeDof.Instance.Recenter(); });
        }
        ...
        private void Update()
        {
            ...
            infoTxt.text = string.Format("Position:{0}\r\nEuler:{1}\r\nRotation:{2}",
                mainCamera.transform.position.ToString("f3"), mainCamera.transform.rotation.eulerAngles.ToString(),
                mainCamera.transform.rotation.ToString("f3"));

            engineTxt.text =
                $"DebugInfo:{NativeInterface.NativeAPI.GetHeadTrackingStatus()},{NativeInterface.NativeAPI.GetDebugInfo()}";


            NativeInterface.NativeAPI.GetSLAMQuality(out var trackingStatus, out var imageQuality,
                out var kineticQuality);
