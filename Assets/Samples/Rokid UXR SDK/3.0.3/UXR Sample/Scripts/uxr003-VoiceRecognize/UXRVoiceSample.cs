...
namespace Rokid.UXR.Demo
{
    public class UXRVoiceSample : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            initVoice();
            OfflineVoiceModule.Instance.AddInstruct(LANGUAGE.ENGLISH, "Show blue", "show blue", gameObject.name,
                "OnReceive");
            OfflineVoiceModule.Instance.AddInstruct(LANGUAGE.ENGLISH, "Show green", "show green", gameObject.name,
                "OnReceive");
            OfflineVoiceModule.Instance.AddInstruct(LANGUAGE.ENGLISH, "Go back", "go back", gameObject.name,
                "OnReceive");

            OfflineVoiceModule.Instance.AddInstruct(LANGUAGE.CHINESE, "回到上一级", "hui dao shang yi ji", gameObject.name,
                "OnReceive");

            OfflineVoiceModule.Instance.AddInstruct(LANGUAGE.CHINESE, "变成蓝色", "bian cheng lan se", gameObject.name,
                "OnReceive");
            OfflineVoiceModule.Instance.AddInstruct(LANGUAGE.CHINESE, "变成绿色", "bian cheng lv se", gameObject.name,
                "OnReceive");
